				...
				<ul class="pagination">
					<c:choose>
						<c:when test="${requestScope.page.prePage}">
							<li><a href="queryStudentsByName?currentPage=${requestScope.page.currentPage-1}&studentName=${studentName}">&laquo;</a></li>
						</c:when>
						<c:otherwise>
					...
					<c:forEach begin="1" end="${requestScope.page.totalPage}" var="i">
						<li><a href="queryStudentsByName?currentPage=${i}&studentName=${studentName}">${i}</a></li>
					</c:forEach>
					
					<c:choose>
						<c:when test="${requestScope.page.nextPage}">
							<li><a href="queryStudentsByName?currentPage=${requestScope.page.currentPage+1}&studentName=${studentName}">&raquo;</a></li>
						</c:when>
						<c:otherwise>
...
<script type="text/javascript">
	$(function(){
		$("ul.pagination a").click(function(){
			...
			var suffixNum = $href.indexOf("studentName")+"studentName".length+1;
			...
			$(this).attr("href","queryStudentsByName?currentPage="+i+"&studentName="+encodeURIComponent(suffix));
		});
	});
</script>
</body>
